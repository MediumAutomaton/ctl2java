...
class CTLConv:
    def __init__(self, infile, requiredFields):
        ...
        try:
            ...
            Common.error("Initial file conversion failed!", True)
        ...
        for key in assets.actionTypes.keys():
            try:
                ...
                if key == "Parameters":
                    ...
                    Common.error("Malformed Action mapping for Primitive '" + buttonName + "' modifier '" + modifier + "'.")
        ...
        if parameters:
            for paramName in parameters.keys():
                ...
                if not isinstance(param, dict):
                    Common.error("Parameter '" + paramName + "' of Action mapped to Primitive '" + buttonName + "' modifier '" + modifier + "' is not a dictionary.")
                ...
                for key in assets.parametersTypes.keys():
                    if key not in param.keys():
                        if key == "Range":
                            ...
                            Common.error("Malformed Parameter '" + paramName + "' of Action mapped to Primitive '" + buttonName + "' modifier '" + modifier + "'.")

                # Type is a valid Java data type
                type = param["Type"]
                if type not in assets.validDataTypes:
                    Common.error("Invalid data type for Parameter '" + paramName + "' of Action mapped to Primitive '" + buttonName + "' modifier '" + modifier + "'.")
    ...
    def getVerifiedDict(self):
        ...
        for item in self.requiredFields:
            if not item in fields:
                Common.error("Missing required field '" + item + "'.")
        ...
        if "Gamepad" in fields:
            ...
            if "Buttons" in fields or "Axes" in fields:
                Common.error("Gamepad mappings must specify which Gamepad they apply to with the 'Gamepad' field!")
        ...
        if "Modifiers" in fields:
            for modifier in self.outdict["Modifiers"]: # Field type already verified in Pass 1
                if modifier in assets.validButtons:
                    ...
                    Common.error("Invalid modifier '" + str(modifier) + "'.")
        else:
        ...
        if "Buttons" in fields and "Buttons" not in emptyFields:
            ...
            for idx, buttonName in enumerate(buttons.keys()):
                ...
                if buttonName in modifiers: # 'modifiers' was set in Pass 2
                    Common.error("Modifier button '" + buttonName + "' should not have Action mappings!")

                button = buttons[buttonName]

                # Button is a dict
                if not isinstance(button, dict):
                    Common.error("Button '" + buttonName + "' is not valid.")
                ...
                for modifier in buttonModifiers:
                    ...
                    if modifier not in (modifiers + ["Default"]):
                        ...
                        if not isinstance(mapping, dict):
                            Common.error("Action mapping for button '" + buttonName + "' modifier '" + modifier + "' is not a dict!")

                        # Mapping contains the correct keys
                        if not mapping.keys() == assets.buttonMappingTypes.keys():
                            Common.error("Malformed mapping for button '" + buttonName + "' modifier '" + modifier + "'.")
                            ...
                            if not isinstance(mapping[key], assets.buttonMappingTypes[key]):
                                ...
                                if mapping[key] == "":
                                    ...
                                    if key == "Action" and isinstance(mapping["Action"], list):
                                        ...
                                        Common.error("Value for key '" + str(key) + "' in modifier mapping '" + modifier + "' in button '" + buttonName + "' is of the wrong type.")
        ...
        if "Axes" in fields and "Axes" not in emptyFields:
            ...
            for idx, axisName in enumerate(axes.keys()):
                ...
                if not isinstance(axis, dict):
                    Common.error("Axis '" + axisName + "' is not valid.")
                ...
                for modifier in axisModifiers:
                    ...
                    if modifier not in (modifiers + ["Default"]):
                        ...
                        # Mapping is a dict
                        if not isinstance(mapping, dict):
                            Common.error("Action mapping for Axis '" + axisName + "' modifier '" + modifier + "' is not a dict!")
                        ...
                        for key in assets.axisMappingTypes.keys():
                            ...
                            try:
                                ...
                                Common.error("Malformed mapping for Axis '" + axisName + "' modifier '" + modifier + "'.")
                        ...
                        if not mapping["Type"] in assets.validAxisTypes:
                            if mapping["Type"] == "":
                                ...
                                Common.error("Invalid Type set for Axis '" + axisName + "' modifier '" + modifier + "'.")
        ...
        if "Actions" in fields and "Actions" not in emptyFields:
            ...
            for actionName in actions.keys():
                ...
                if not isinstance(action, dict):
                    Common.error("Action '" + actionName + "' is not a dict.")

                # Action has all required fields
                for requiredField in assets.libActionRequiredFields:
                    if not requiredField in action.keys():
                        Common.error("Action '" + actionName + "' is missing required field '" + requiredField + "'.")

                # Verify Type (required)
                type = action["Type"]
                if not isinstance(type, str):
                    Common.error("'Type' field for Action '" + actionName + "' is not a string.")
                if not type in assets.validLibActionTypes:
                    Common.error("'" + type + "' for Action '" + actionName + "' is not a valid Action type!")
                print("Type")

                # Verify Code (required)
                code = action["Code"]
                if not isinstance(code, list):
                    Common.error("'Code' field for Action '" + actionName + "' is not a list.")
                for idx, line in enumerate(code):
                    if not isinstance(line, str):
                        Common.error("Line [" + str(idx) + "] of Code for Action '" + actionName + "' is not a string.")
                print("Code")
                ...
                if "Parameters" in action.keys():
                    ...
                    if not isinstance(parameters, dict):
                        Common.error("'Parameters' for Action '" + actionName + "' is not a dict.")
                    ...
                    for paramName in parameters.keys():
                        ...
                        # Must be a dict
                        if not isinstance(param, dict):
                            Common.error("Parameter '" + paramName + "' of Action '" + actionName + "' is not a dict.")

                        # Check for required keys
                        for key in assets.libActionParamFields:
                            if not key in param.keys():
                                Common.error("Parameter '" + paramName + "' of Action '" + actionName + "' is missing required field '" + key + "'.")
                        ...
                        type = param["Type"]
                        if not isinstance(type, str):
                            Common.error("Type of Parameter '" + paramName + "' of Action '" + actionName + "' is not a string.")
                        if not type in assets.validDataTypes:
                            Common.error("Type of Parameter '" + paramName + "' of Action '" + actionName + "' is not a valid Java data type.")
        ...
        if "Methods" in fields and "Methods" not in emptyFields:
            if not ("Bases" in fields or "Bases" in emptyFields):
                Common.error("Method library is missing 'Bases' field.")
            if not ("Extensions" in fields or "Extensions" in emptyFields):
                Common.error("Method library is missing 'Extensions' field.")
            ...
            for methodName in methods.keys():
                ...
                if not isinstance(method, list):
                    Common.error("Method '" + methodName + "' is not a list.")

                for idx, line in enumerate(method):
                    if not isinstance(line, str):
                        Common.error("Line [" + str(idx) + "] of Method '" + methodName + "' is not a string.")
        else:
        ...
        if "Setters" in fields and "Setters" not in emptyFields:
            ...
            for idx, line in enumerate(setters):
                if not isinstance(line, str):
                    Common.error("Line [" + str(idx) + "] of Setters is not a string.")
        else:
